	...
	if !err {
		...
		if errors.Is(dberr, gorm.ErrRecordNotFound) {
			...
			c.JSON(http.StatusOK, gin.H{"data": university, "source": source})
				...
				logger.Printf(fmt.Sprintf("[%s] Data retrieved from %s\n", time.Now().Format("2006/01/02 - 15:04:05"), source))
		...
		c.JSON(http.StatusOK, gin.H{"data": university, "source": source})
			...
			logger.Printf("[%s] Data retrieved from Cache\n", time.Now().Format("2006/01/02 - 15:04:05"))
...
func deleteUniversity(c *gin.Context) {
    ...
    rankupdate, err := strconv.Atoi(c.Param("rank"))
    ...
    if err := DB.First(&university, rankupdate).Error; err != nil {

        logger.Printf("University Not Found for rank %d", rankupdate)
    ...
    if err := DB.Delete(&University{}, c.Param("rank")).Error; err != nil {
        ...
        logger.Printf("Failed to delete university with rank %d", rankupdate)
    ...
    logger.Printf("Successfully deleted university with rank %d", rankupdate)

    c.JSON(http.StatusOK, gin.H{"Status": "University Deleted succcessfully!!"})
...
func updateUniversity(c *gin.Context) {
    ...
    rankupdate, _ := strconv.Atoi(c.Param("rank"))

    if err := DB.Where(University{Rank:uint16(rankupdate)}).First(&institution).Error; err != nil {

        log.Printf("University Not Found for rank %d", rankupdate)
    ...
    if err := c.ShouldBindJSON(&input); err != nil {
        ...
        log.Printf("Unable to update university for rank %d", rankupdate)
    ...
    logger.Printf("Successfully updated university for rank %d", rankupdate)
